...
public class GetRandomQuestionController {
    ...
    @RequestMapping(value="/GetQuestion.from", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
    public @ResponseBody
    String getRandomQuestion(HttpServletRequest request, HttpServletResponse response) {
        log.info("开始随机获取问题-------");
        try {
            List<Question> questionList=questionService.getRandomQuestionById();
            int size=questionList.size();
            String result="";
            for(int i=0;i<size;i++){
                Question q=questionList.get(i);
                result+=""+q.getQuestion_id()+"@%"+q.getQuestion_title()+"@%"+q.getQuestion_selection()+"@$%";
            }
            return result;
        }catch (Exception e){
