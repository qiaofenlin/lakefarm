...
import com.lakefarm.mapper.ShopGrowplantsMapper;
...
//        String zzIdStr = request.getParameter("zzId");
        ...
        if (plantCount!=0){
        GrowPlantsCustom growPlantsCustom =growplantsShopService.selectGrowplantInfo(zzId);
        if(null == growPlantsCustom){
            log.info("种子id异常！"); // todo 种子id异常
            return "0";
        }else {
           log.info("种子信息为："+ growPlantsCustom);


           Integer u_money =employeeMapper.findUserMoneyById(u_idStr);
            log.info("计算余额是否能够购买");
            int balance=0;
            if((balance=(u_money- growPlantsCustom.getZzPrice()*zz_allcountBuy))>=0){
                ...
                log.info("更新背包");
                PersonGrowplantsCustom personGrowplantsCustom = new PersonGrowplantsCustom();
                personGrowplantsCustom.setuId(u_id);
                personGrowplantsCustom.setZzId(zzId);
                if(null != zz_allcount){
                    zz_allcount = zz_allcountBuy+zz_allcount;
                    personGrowplantsCustom.setZzCount(zz_allcount);
                    try {
                        personGrowplantsMapper.updateByPrimaryKey(personGrowplantsCustom);
                        log.info("成功更新数据到背包！");
                    }catch (Exception e){
                    ...
                    personGrowplantsCustom.setZzCount(zz_allcount);
                    try {
                        personGrowplantsMapper.insert(personGrowplantsCustom);
                        log.info("成功插入数据到背包！");
                    }
                    catch (Exception e){
